...
def extract_dominant_color_from_image_bytes(image_bytes: bytes, crop_zone: List[int]) -> List[int]:
    """Extract dominant container color from byte image with cropping."""
    ...
    if crop_x2 <= crop_x1 or crop_y2 <= crop_y1:
        log.warning("Crop zone resulted in empty region (x=%d-%d, y=%d-%d) on %dx%d image", crop_x1, crop_x2, crop_y1, crop_y2, w, h)
